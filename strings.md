...
**Ответ:**  
```java
Pattern p = Pattern.compile("(http|https)\\S+?(\\.by)");
Matcher m = p.matcher(belGutHtml);
while (m.find()) {
...
Matcher m = p.matcher("https://bsut.by?value1=5&value2=10&iscan=true");
while (m.find()) {
...
\\s+ s - встречается 1 или более раз = false т.к. пробел будет принят предыдущей проверкой \\s*
\\S{4} - 4 непробельных символа = true
