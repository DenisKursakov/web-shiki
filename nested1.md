...
а) Чтобы создать экземпляр внутреннего класса, вы должны сначала создать экземпляр внешнего класса. Затем создайте внутренний объект внутри внешнего объекта с помощью этого синтаксиса:  
Однако создать экземпляр статического вложенного класса можно так же, как и класс верхнего уровня:
```java  
NestedPublic nestedObject = new NestedPublic;
Outer.InnerPublic innerObject = outerObject.new InnerPublic ();
```
...
**Ответ:**  
а) К экземпляру класс  ```Inner``` можно обратиться через ключевое слово ```this```
...
в роли объектов наследования. Допустимо наследование следующего вида:
```java 
...
class Runner {
    ...
    public static void main(String[] args) {
        new Runner().new Outer().new Inner().go(); //1
    }
}
...
С помощью ```Java-8``` вы можете определить основной метод внутри интерфейса, приведенный ниже код будет работать в ```Java-8```:
```java 
