...
(23) Можно ли выполнить операции:  
1: ```b = i;``` ?  
2: ```i = b;``` ?  
**Ответ: (21,22,23)**   
Класс А - абстрактный класс, соответственно мы не можем создать объект класса А.
...
его супер класс А. Так же допустимо объявление ссылки на интерфейсный тип или использование ее
в качестве параметра метода. Такая ссылка может указывать на экземпляр любого класса, который реализует объявленный интерфейс. При вызове метода
через такую ссылку будет вызываться его реализованная версия, основанная на текущем экземпляре класса.Рассмотрим пример инициализации ссылки на абстрактный суперкласс:
```java 
public abstract class AbstractCardAction { //create abstract class
...
…
public class CreditCardAction extends AbstractCardAction { //create extending class
...
action.check();
```
Рассмотрим свойства ссылок на интерфейс I с участием класса B который реализует наш интерфейс I:
```java 
public class Main {
...
i = new B();
``` 
Исходя из примера мы видим что i = b допустимое равенство, однако b = i - недопустимое т.к. данное сравнение подразумевает идентичность типов.  
**Источник:** И.Н. Блинов стр 134, стр 184  
https://coderoad.ru/4989818/instanceof-Vs-getClass#4989843  
(24) В чем заключается отличие сравнения принадлежности к классу через операцию
...
4: Нет, в языке программирования Java интерфейс - это ссылочный тип, подобный классу, который может содержать только константы, сигнатуры методов, методы по умолчанию, статические методы и вложенные типы. Тела методов существуют только для методов по умолчанию и статических методов.
...
(27) Для каких целей используется расширение интерфейса?  
...
(28) Могут ли в интерфейсе быть поля?  
...
**Источник:** И.Н.Блинов стр. 179  
(29) Можно ли в интерфейсе  
...
(30) Можно ли интерфейс объявить финальным? Поясните ответ.  
**Ответ:**
Интерфейсы на 100% абстрактны. Разрешать интерфейсам быть final совершенно бессмысленно.  
(31) Можно ли в интерфейсе объявить статический метод? Поясните ответ.  
...
(32) Можно ли вызвать статический метод через объектную ссылку? Поясните ответ.  
**Ответ:**
...
**Источник:** И.Н. Блинов стр. 83.  
(33) Можно ли создать экземпляр класса, у которого есть абстрактный метод? Если да, то
зачем?  
**Ответ:**
...
(34) Должен ли иметь абстрактный класс хотя бы один абстрактный метод? Если нет, то
...
**Источник:** Блинов стр. 134.  
(35) Что означает ключевое слово final в следующих конструкциях?  
1.
```java 
...
(36) Можно ли объявить метод одновременно финальным и абстрактным? Поясните ответ.  
...
(37) К каким методам неприменимы принципы позднего связывания? Почему?  
...
(38) В чем заключается отличие между ранним и поздним связыванием?  
**Ответ:** 
